/**
 * UBS-MEM noncache load benchmark.
 *
 * Maps the server-created "<name>_nc" object and reads it with CPU
 * loads/memcpy. In this mode loads should pierce local CPU cache and expose
 * the remote fabric read latency.
 */
...
#define BENCH_TITLE "UBS-MEM Noncache Read Benchmark"
#define MODE_NAME "UBS-MEM (noncache)"
#define UBSMEM_FLAGS UBSM_FLAG_NONCACHE
...
{
    snprintf(out, out_len, "%s_nc", base);
}
...
int main(int argc, char **argv)
{
    ...
    printf("shmem: %s_nc, size: %zu MB, iters: %d, provider: %s\n",
