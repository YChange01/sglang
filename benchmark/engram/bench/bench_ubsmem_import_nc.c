/**
 * UBS-MEM import-noncache load benchmark.
 *
 * Maps the server-created "<name>_import_nc" object and reads it with CPU
 * loads/memcpy. This mode keeps the object allocation anonymous on the
 * provider side and makes the imported mapping noncacheable on the consumer
 * side, matching the common UB-M-NC puncture setup more closely than full
 * UBSM_FLAG_NONCACHE on both sides.
 */
...
#define BENCH_TITLE "UBS-MEM Import-Noncache Read Benchmark"
#define MODE_NAME "UBS-MEM (import noncache)"
#define UBSMEM_FLAGS (UBSM_FLAG_ONLY_IMPORT_NONCACHE | UBSM_FLAG_MEM_ANONYMOUS)
...
{
    snprintf(out, out_len, "%s_import_nc", base);
}
...
int main(int argc, char **argv)
{
    ...
    printf("shmem: %s_import_nc, size: %zu MB, iters: %d, provider: %s\n",
