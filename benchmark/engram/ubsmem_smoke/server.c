...
int main(int argc, char *argv[])
{
    ...
    if (ubsmem_lookup_cluster_statistic(&cinfo) == 0) {
        ...
        printf("[2/5] cluster query failed (non-fatal)\n");
    }
    ...
    printf("[3/5] ubsmem_shmem_allocate OK (name=%s, size=%zu)\n",
           shm_name, buf_size);
    ...
    ret = ubsmem_shmem_map(NULL, buf_size,
                           ...
                           shm_name, 0, &ptr);
    if (ret != 0 || !ptr) {
        fprintf(stderr, "[4/5] ubsmem_shmem_map failed: %d\n", ret);
        ubsmem_shmem_deallocate(shm_name);
        ubsmem_finalize();
        return 1;
    }
    printf("[4/5] ubsmem_shmem_map OK: ptr=%p\n", ptr);

    /* Step 5: Fill with deterministic pattern — same as scheme5 server */
    float *data = (float *)ptr;
    size_t num_floats = buf_size / sizeof(float);
    printf("[5/5] Filling %zu floats (data[i] = i * 0.001f)...\n", num_floats);
